...
from lib_land_registry_data.lib_constants.process_name import PROCESS_NAME_LAND_REGISTRY_DATA_PP_COMPLETE_DATA_DECISION
...
from lib_land_registry_data.lib_topic_name import TOPIC_NAME_LAND_REGISTRY_DATA_PP_COMPLETE_DATA_DECISION_NOTIFICATION
...
set_logger_process_name(
    process_name=PROCESS_NAME_LAND_REGISTRY_DATA_PP_COMPLETE_DATA_DECISION,
)

logger = get_logger()
stdout_log_handler = create_stdout_log_handler()
file_log_handler = create_file_log_handler(
    logger_process_name=PROCESS_NAME_LAND_REGISTRY_DATA_PP_COMPLETE_DATA_DECISION,
...
def main():
    logger.info(f'{PROCESS_NAME_LAND_REGISTRY_DATA_PP_COMPLETE_DATA_DECISION} start')
    ...
    consumer = create_consumer(
        ...
        client_id=PROCESS_NAME_LAND_REGISTRY_DATA_PP_COMPLETE_DATA_DECISION,
        group_id=PROCESS_NAME_LAND_REGISTRY_DATA_PP_COMPLETE_DATA_DECISION,
    )
...
def kafka_event_loop(
    ...
    logger.info(f'consumer subscribing to topic {TOPIC_NAME_LAND_REGISTRY_DATA_PP_COMPLETE_DOWNLOAD_NOTIFICATION}')
    consumer.subscribe([TOPIC_NAME_LAND_REGISTRY_DATA_PP_COMPLETE_DOWNLOAD_NOTIFICATION])
...
def notify(
    ...
    dto = PPCompleteDataDecisionNotificationDTO(
        notification_source=PROCESS_NAME_LAND_REGISTRY_DATA_PP_COMPLETE_DATA_DECISION,
    ...
    producer.produce(
        topic=TOPIC_NAME_LAND_REGISTRY_DATA_PP_COMPLETE_DATA_DECISION_NOTIFICATION,
        key=f'no_key',
        value=dto_json_str,
    )
    producer.flush()
