...
from lib_land_registry_data.lib_constants.process_name import PROCESS_NAME_PP_MONTHLY_UPDATE_GARBAGE_COLLECTOR

from lib_land_registry_data.lib_constants.topic_name import TOPIC_NAME_PP_MONTHLY_UPDATE_DATA_DECISION_NOTIFICATION
from lib_land_registry_data.lib_constants.topic_name import TOPIC_NAME_PP_MONTHLY_UPDATE_GC_NOTIFICATION
...
from lib_land_registry_data.lib_constants.notification_type import NOTIFICATION_TYPE_PP_MONTHLY_UPDATE_GC_COMPLETE

from lib_land_registry_data.lib_dto import PPMonthlyUpdateDataDecisionNotificationDTO
from lib_land_registry_data.lib_dto import PPMonthlyUpdateGCNotificationDTO
...
set_logger_process_name(
    process_name=PROCESS_NAME_PP_MONTHLY_UPDATE_GARBAGE_COLLECTOR,
)

logger = get_logger()
stdout_log_handler = create_stdout_log_handler()
file_log_handler = create_file_log_handler(
    logger_process_name=PROCESS_NAME_PP_MONTHLY_UPDATE_GARBAGE_COLLECTOR,
...
def main():
    logger.info(f'{PROCESS_NAME_PP_MONTHLY_UPDATE_GARBAGE_COLLECTOR} start')
    ...
    consumer = create_consumer(
        ...
        client_id=PROCESS_NAME_PP_MONTHLY_UPDATE_GARBAGE_COLLECTOR,
        group_id=PROCESS_NAME_PP_MONTHLY_UPDATE_GARBAGE_COLLECTOR,
    )
...
def notify(
    ...
    dto = PPMonthlyUpdateGCNotificationDTO(
        notification_source=PROCESS_NAME_PP_MONTHLY_UPDATE_GARBAGE_COLLECTOR,
        notification_type=NOTIFICATION_TYPE_PP_MONTHLY_UPDATE_GC_COMPLETE,
    ...
    producer.produce(
        topic=TOPIC_NAME_PP_MONTHLY_UPDATE_GC_NOTIFICATION,
        key=f'no_key',
        value=dto_json_str,
    )
    producer.flush()
