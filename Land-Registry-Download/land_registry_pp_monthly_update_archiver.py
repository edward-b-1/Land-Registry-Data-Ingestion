...
set_logger_process_name(
    process_name=PROCESS_NAME_PP_MONTHLY_UPDATE_ARCHIVER,
)

logger = get_logger()
stdout_log_handler = create_stdout_log_handler()
file_log_handler = create_file_log_handler(
    logger_process_name=PROCESS_NAME_PP_MONTHLY_UPDATE_ARCHIVER,
...
def main():
    logger.info(f'{PROCESS_NAME_PP_MONTHLY_UPDATE_ARCHIVER} start')
    ...
    consumer = create_consumer(
        ...
        client_id=PROCESS_NAME_PP_MONTHLY_UPDATE_ARCHIVER,
        group_id=PROCESS_NAME_PP_MONTHLY_UPDATE_ARCHIVER,
    )
...
def archive(
    ...
    with Session(postgres_engine) as session:
        ...
        logger.info(f'querying database table PPMonthlyUpdateDownloadFileLog for rows with process_decision=\'archive\'')
        ...
        for row in rows:
            ...
            if data_decision == 'archive':
                ...
                s3_copy_start_datetime = datetime.now(timezone.utc)
                ...
                s3_copy_complete_datetime = datetime.now(timezone.utc)
                s3_copy_duration = s3_copy_complete_datetime - s3_copy_start_datetime

                row.s3_archive_action_taken = 'archive'
                row.s3_archive_datetime = datetime.now(timezone.utc)
                row.s3_copy_start_datetime = s3_copy_start_datetime
